...
branch<-args[1]
rm(all)
nsign<-nsign[nsign$branch==branch,]
...
spSign<-merge(nsign[,1:5],brdummy)
...
for(i in 1:nrow(brSign)){
	...
	if(nrow(in_tspSign)>1 & nrow(out_tspSign)>1){
		...
		nsign$inD[nsign$start==tspSign$start[1] & nsign$end==tspSign$end[1]]<-mean(in_d)
		nsign$outD[nsign$start==tspSign$start[1] & nsign$end==tspSign$end[1]]<-mean(out_d)
		nsign$allD[nsign$start==tspSign$start[1] & nsign$end==tspSign$end[1]]<-mean(all_d)
		nsign$crossD[nsign$start==tspSign$start[1] & nsign$end==tspSign$end[1]]<-mean(as.vector(all_d)[as.vector(all_d) %out% c(as.vector(in_d),as.vector(out_d))])
}}
write.table(nsign[!is.na(nsign$inD),],
	gzfile(paste("fimo-nsign-",branch,".tsv.gz",sep="")),
