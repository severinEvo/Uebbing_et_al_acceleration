...
foreach my $i (@chroms){
	...
	print OUT "maf_parse -g tmp_pCE_$i\.bed pCEs_$i\.maf >tmp_mafs/\"\$j\"\\.maf; ";
	print OUT "done; ";
	...
	print OUT "maf_parse -g tmp_cCRE_$i\.bed cCREs_$i\.maf >tmp_mafs/\"\$j\"\\.maf; ";
	print OUT "done; ";
...
# Create jobs:
# module load dSQ
# dsq --job-file PRE-MAFs.batch --mem-per-cpu 6G --mail-type FAIL,END
